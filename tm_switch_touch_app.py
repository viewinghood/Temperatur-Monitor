# -*- coding: utf-8 -*-
"""
Touch-friendly launcher: switch from HDMI (Eizo) to 7" DSI touch and reboot.

Run: ~/py/TempMonitor/dev/start_tm_switch_touch.sh
Desktop: SwitchToTouch.desktop
"""
...
from tm_display_switch import apply_touch_switch_and_reboot
...
class SwitchTouchApp(App):
    ...
    def build(self):
        ...
        if is_touch_display_mode():
            root.add_widget(Label(
                text='Bereits 7"-Touch-Display aktiv.\nKein Umschalten noetig.',
                font_size='18sp',
                color=MUTED_COLOR,
                halign='center',
            ))
        ...
        hint = Label(
            text='HDMI (Eizo) aktiv.\nAuf das 7"-Touch-Display umschalten?',
            font_size='17sp',
            color=MUTED_COLOR,
            halign='center',
        )
